...
CONTAINS
   ...
   SUBROUTINE trc_rst_read( Kbb, Kmm )
      ...
      !tr(:,:,:,jpcdz,Kbb) = tr(:,:,:,jpphy,Kbb)*0.1
      !tr(:,:,:,jpcdz,Kmm) = tr(:,:,:,jpcdz,Kbb)
      !tr(:,:,:,jpfed,Kbb) = tr(:,:,:,jpnfe,Kbb)*0.1
      !tr(:,:,:,jpfed,Kmm) = tr(:,:,:,jpfed,Kbb)
      !tr(:,:,:,jpchd,Kbb) = tr(:,:,:,jpnch,Kbb)*0.1
      !tr(:,:,:,jpchd,Kmm) = tr(:,:,:,jpchd,Kbb)
      !tr(:,:,:,jpndz,Kbb) = tr(:,:,:,jpnph,Kbb)*0.1
      !tr(:,:,:,jpndz,Kmm) = tr(:,:,:,jpndz,Kbb)
      !tr(:,:,:,jppdz,Kbb) = tr(:,:,:,jppph,Kbb)*0.1
      !tr(:,:,:,jppdz,Kmm) = tr(:,:,:,jppdz,Kbb)
      !trn(:,:,:,jppdz) = trb(:,:,:,jppdz)
!      tr(:,:,:,jpdz1,Kbb) = 0._wp
!      tr(:,:,:,jpdz1,Kmm) = tr(:,:,:,jpdz1,Kbb)
!      tr(:,:,:,jpdz2,Kbb) = 0._wp
!      tr(:,:,:,jpdz2,Kmm) = tr(:,:,:,jpdz2,Kbb)
      !
