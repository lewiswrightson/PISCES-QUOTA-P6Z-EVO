MODULE p4zbc
   ...
   TYPE(FLD), ALLOCATABLE, DIMENSION(:) ::   sf_solfe     !
   TYPE(FLD), ALLOCATABLE, DIMENSION(:) ::   sf_lith      !
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   solfe    !: dust
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   lith
...
CONTAINS
   ...
   SUBROUTINE p4z_bc( kt, Kbb, Kmm, Krhs )
      ...
      IF( ll_dust )  THEN
         ...
         CALL fld_read( kt, 1, sf_solfe )
            solfe(:,:) = MAX( rtrn, sf_solfe(1)%fnow(:,:,1) )
         ENDIF

         IF( ln_felith) THEN
         CALL fld_read( kt, 1, sf_lith )
            lith(:,:) = MAX( rtrn, sf_lith(1)%fnow(:,:,1) )
         ENDIF
         !
         ...
         IF( .NOT. ln_felith ) THEN ! only do subsurface dust dissolution when the
            !
         ...
         ENDIF
            IF( lk_iomput ) THEN
                ...
                IF( ln_felith ) THEN
                jl = n_trc_indsbc(jplfe)
                CALL iom_put( "lith", ( rf_trsfac(jl) * sf_trcsbc(jl)%fnow(:,:,1) / rn_sbc_time ) * 1.e+3 * tmask(:,:,1) )
                ENDIF
            ENDIF
         ...
         IF( ln_felith ) THEN
            DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 2, jpkm1 )
               tr(ji,jj,jk,jpfer,Krhs) = tr(ji,jj,jk,jpfer,Krhs) + ( tr(ji,jj,jk,jplfe,Kbb) &
               &     * ( sollfe / tsollfe ) * rfact )
               tr(ji,jj,jk,jplfe,Krhs) = tr(ji,jj,jk,jplfe,Krhs) - ( tr(ji,jj,jk,jplfe,Kbb) &
               &     * ( sollfe / tsollfe ) * rfact )
            END_3D
         ENDIF
   ...
   SUBROUTINE p4z_bc_init( Kmm ) 
      ...
      TYPE(FLD_N) ::   sn_dust, sn_lith, sn_ironsed, sn_hydrofe, sn_solfe   ! informations about the fields to be read
      !!
      NAMELIST/nampisbc/cn_dir, sn_dust, sn_lith, sn_ironsed, sn_hydrofe, sn_solfe, &
        &                ln_ironsed, ln_ironice, ln_hydrofe,     &
        &                sedfeinput, distcoast, icefeinput, wdust, mfrac,   &
        &                hratio, lgw_rath, sollfe, tsollfe, flfe
      !!----------------------------------------------------------------------
      !
      IF(lwp) THEN
         WRITE(numout,*)
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nampisbc in configuration namelist' )
      ...
      IF( ln_bait ) THEN
         ...
         IF(lwp) WRITE(numout,*) '    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ '
         !
         ALLOCATE( solfe(jpi,jpj) )
         !
         ALLOCATE( sf_solfe(1), STAT=ierr )
         IF( ierr > 0 )   CALL ctl_stop( 'STOP', 'p4z_bc_init: unable to allocate sf_solfe structure' )
         CALL fld_fill( sf_solfe, (/ sn_solfe /), cn_dir, 'p4z_bc_init', 'Atmospheric solfe deposition', 'nampisbc' )
                                   ALLOCATE( sf_solfe(1)%fnow(jpi,jpj,1)   )
         IF( sn_solfe%ln_tint )    ALLOCATE( sf_solfe(1)%fdta(jpi,jpj,1,2) )
      END IF

      IF( ln_felith ) THEN
         IF(lwp) WRITE(numout,*) '    initialize lithogenics input from atmosphere '
         IF(lwp) WRITE(numout,*) '    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ '
         !
         ALLOCATE( lith(jpi,jpj) )
         !
         ALLOCATE( sf_lith(1), STAT=ierr )
         IF( ierr > 0 )   CALL ctl_stop( 'STOP', 'p4z_bc_init: unable to allocate sf_lith structure' )
         CALL fld_fill( sf_lith, (/ sn_lith /), cn_dir, 'p4z_bc_init', 'Atmospheric lith deposition', 'nampisbc' )
                                   ALLOCATE( sf_lith(1)%fnow(jpi,jpj,1)   )
         IF( sn_lith%ln_tint )     ALLOCATE( sf_lith(1)%fdta(jpi,jpj,1,2) )
      END IF
