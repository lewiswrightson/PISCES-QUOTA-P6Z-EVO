...
CONTAINS
   ...
   SUBROUTINE p4z_ligand( kt, knt, Kbb, Krhs )
      ...
      IF (ln_bait) THEN
         zlgwr = max( rlgs , rlgw * exp( -2 * (tr(ji,jj,jk,jplgw,Kbb) * 1.0E9) ) ) ! years
      ...
      IF (ln_bait) THEN
        ! In this scheme, then any lgw complexed with Fe is by definition
        ! soluble and not colloidal - so subtract proportion of FeL1 from lgw
        ! clgw is the colloidal fraction of regular ligands
         zligco   = clgw * max( 0., tr(ji,jj,jk,jplgw,Kbb) -  &
         &                    ( plig(ji,jj,jk) * tr(ji,jj,jk,jpfer,Kbb) ) )
   ...
   SUBROUTINE p4z_ligand_init
      ...
      !
      NAMELIST/nampislig/ rlgw, prlgw, rlgs, rlig, xklig, clgw
      !!----------------------------------------------------------------------
      !
      IF(lwp) THEN
         WRITE(numout,*)
