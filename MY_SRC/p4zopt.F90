...
CONTAINS
   ...
   SUBROUTINE p4z_opt( kt, knt, Kbb, Kmm )
      ...
      DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, jpkm1)
         ...
         IF( ln_p6z )   zchl = zchl + (tr(ji,jj,jk,jppch,Kbb) + tr(ji,jj,jk,jpchd,Kbb))  * 1.e6
      ...
      ELSE   ! no diurnal cycle
         ...
         IF( ln_p6z ) THEN
            DO jk = 1, nksr
              ediaz(:,:,jk) =  1.85 * ze1(:,:,jk) + 0.69 * ze2(:,:,jk) + 0.46 *ze3(:,:,jk)  ! Explicit diazotroph (PISCES-QUOTA)
            END DO
         ENDIF
         etot_ndcy(:,:,:) =  etot(:,:,:) 
      ENDIF
      ...
      IF( ln_p6z) THEN
         ...
         ! Explicit diazotroph when using PISCES-QUOTA
         ALLOCATE( zetmp5(jpi,jpj) )  ;   zetmp5 (:,:) = 0.e0
         DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, nksr)
            IF( gdepw(ji,jj,jk+1,Kmm) <= MIN(hmld(ji,jj), heup_01(ji,jj)) ) THEN
