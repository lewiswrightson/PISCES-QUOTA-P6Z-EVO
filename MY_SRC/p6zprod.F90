MODULE p6zprod
   ...
   REAL(wp), PUBLIC ::  thetandzm        !: Maximum Chl/N ratio of diazotrophs
   
   REAL(wp), PUBLIC ::  evotime        !: Adaptation timescale for diazotrophs
   ...
   REAL(wp), PUBLIC ::  qfemaxdz          !:
   REAL(wp), PUBLIC ::  kqfemaxdz          !:
...
CONTAINS
   ...
   SUBROUTINE p6z_prod( kt , knt, Kbb, Kmm, Krhs )
      ...
      REAL(wp) ::   zpronfmax, facul, zqfdzmax
      ...
      ! Diazotrophy
      REAL(wp), DIMENSION(jpi,jpj    ) :: zmixdiaz
      ...
      REAL(wp), DIMENSION(jpi,jpj,jpk) :: zpropo4dz, zprodopdz
      REAL(wp), DIMENSION(jpi,jpj,jpk) :: znfix, zprnfix, zprnutdz,qn_diazo, nutlim_dz, Facul_out
      REAL(wp), DIMENSION(jpi,jpj,jpk) :: zprnfix1, zprnfix2, zprmaxdz1, zprmaxdz2
      REAL(wp), DIMENSION(jpi,jpj,jpk) :: zprnutdz1, zprnutdz2
      ...
      znfix(:,:,:) = 0._wp ; zprnfix1(:,:,:) = 0._wp ; zpronewdz(:,:,:) = 0._wp
      zpronewdz(:,:,:) = 0._wp ; zproregdz(:,:,:) = 0._wp ; zpropo4dz(:,:,:) = 0._wp
      zprodopdz(:,:,:) = 0._wp ; zwork(:,:) = 0._wp 
      Facul_out(:,:,:) = 0._wp ; qn_diazo(:,:,:) =0._wp ; nutlim_dz(:,:,:) = 0._wp
      zprnfix2(:,:,:) = 0._wp  ; zprmaxdz(:,:,:) = 0._wp ; zprmaxdz1(:,:,:) = 0._wp
      zprmaxdz2(:,:,:) = 0._wp
     ...
     ! set diazo growth rate based on Crocosphaera T28
     DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, nksr )
         zprnfix1(ji,jj,jk) = ((-1.3663E-04 * (ts(ji,jj,jk,jp_tem,Kmm)**4))+(0.0144345 * (ts(ji,jj,jk,jp_tem,Kmm)**3)) +  &
         &                   (-0.5731*(ts(ji,jj,jk,jp_tem,Kmm)**2)) + (10.1515 * ts(ji,jj,jk,jp_tem,Kmm))               &
         &                   - 67.4056 ) * r1_rday
         IF(ts(ji,jj,jk,jp_tem,Kmm) .LT. 20 .OR. ts(ji,jj,jk,jp_tem,Kmm) .GT. 34 ) THEN
         zprnfix1(ji,jj,jk) = 0.
         ENDIF
         IF (zprnfix1(ji,jj,jk) .LT. 0) THEN
         zprnfix1(ji,jj,jk) = 0.
         ENDIF
         zprmaxdz1(ji,jj,jk) = (1. + xpsino3 * qndzmax) * zprnfix1(ji,jj,jk) !Optimal production
         zprnutdz1(ji,jj,jk) = zprnfix1(ji,jj,jk) !(1 + xpsino3 * qndzmax) * zprnfix1(ji,jj,jk)  !Optimal nutrient uptake
      END_3D

     ! set diazo growth rate based on Crocosphaera T32
     DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, nksr )
         zprnfix2(ji,jj,jk) = ((-2.4662E-04 * (ts(ji,jj,jk,jp_tem,Kmm)**4))+(0.026078 * (ts(ji,jj,jk,jp_tem,Kmm)**3)) +  &
         &                   (-1.0293 * (ts(ji,jj,jk,jp_tem,Kmm)**2)) + ( 17.9912 * ts(ji,jj,jk,jp_tem,Kmm))          &
         &                    -117.2700) * r1_rday

        IF(ts(ji,jj,jk,jp_tem,Kmm) .LT. 20 .OR. ts(ji,jj,jk,jp_tem,Kmm) .GT. 34 ) THEN
         zprnfix2(ji,jj,jk) = 0.
         ENDIF
         IF (zprnfix2(ji,jj,jk) .LT. 0) THEN
         zprnfix2(ji,jj,jk) = 0.
         ENDIF
         zprmaxdz2(ji,jj,jk) = (1. + xpsino3 * qndzmax) * zprnfix2(ji,jj,jk) !Optimal production
         zprnutdz2(ji,jj,jk) = zprnfix2(ji,jj,jk) !(1 + xpsino3 * qndzmax) * zprnfix2(ji,jj,jk)  !Optimal nutrient uptake
     END_3D

    IF( ln_evolve ) THEN

      ! Reset the Evolution loop
      ! If the 28oC adapted has evolved into the 32oC (T>32 for longer than evotime)
      ! but then the evolved ecotype has spent more than the evotime below 32oC
      ! then it evolves back and the evolution process starts again 

     DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, nksr )      
         IF ( bckevoclock(ji,jj,jk) .GT. evotime ) THEN
         evoclock(ji,jj,jk) = 0
         bckevoclock(ji,jj,jk) = 0
         ELSE
         evoclock(ji,jj,jk) = evoclock(ji,jj,jk)
         ENDIF
     END_3D

      ! If diazotroph has already evolved evoclock >= evotime the the forward
      ! evolution clock stops counting, if not whenever the T > 32 the clock
      ! counts forward, if T<32 the clock counts down. When the T>32 for longer
      ! than the evolution time scale the diazotroph can evolve from 28oC to 32oC ecotype
      !clock if T > T threshold +1 
     DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, nksr )
         IF( evoclock(ji,jj,jk) .GT. evotime ) THEN
         evoclock(ji,jj,jk)= evoclock(ji,jj,jk)

            ! Allow backwards adaptation, once the diazotroph has evolved into the
            ! 32oC ecotype, if T<32 for longer than the evolution time scale the
            ! diazotroph reverts back to 28oC ecotype and the clocks are reset. 
            ! Whenever the T < 32 the backwards clock counts forward counts forward,
            ! if T > 32 the clock counts down.
            IF(ts(ji,jj,jk,jp_tem,Kmm) .LT. 32 ) THEN
               bckevoclock(ji,jj,jk) = bckevoclock(ji,jj,jk) + (1*xstep)
            ELSE
               bckevoclock(ji,jj,jk) = bckevoclock(ji,jj,jk) - (1*xstep)
            ENDIF
            ! ensure backwards evolution clock never goes negative
            bckevoclock(ji,jj,jk) = max(0.,bckevoclock(ji,jj,jk))
         ELSE
           IF(ts(ji,jj,jk,jp_tem,Kmm) .GE. 32 ) THEN
           !tr(ji,jj,jk,jpevo) = tr(ji,jj,jk,jpevo) + (1*xstep)
           evoclock(ji,jj,jk)= evoclock(ji,jj,jk) + (1*xstep)
           ELSE
           !tr(ji,jj,jk,jpevo) = tr(ji,jj,jk,jpevo)
           evoclock(ji,jj,jk)= evoclock(ji,jj,jk)-(1*xstep)
           ENDIF
           ! Ensure forward evolution clock never goes negative
           evoclock(ji,jj,jk) = max(0., evoclock(ji,jj,jk))
         ENDIF
     END_3D

     ! Thermal evolution
     ! if clock is > 2years diazo T32 (diazo2) selected

     DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, nksr )
         IF(evoclock(ji,jj,jk) .GT. evotime ) THEN
         tr(ji,jj,jk,jpdz1,Krhs) = 0
         tr(ji,jj,jk,jpdz2,Krhs) = 1
         ELSE
         tr(ji,jj,jk,jpdz1,Krhs) = 1
         tr(ji,jj,jk,jpdz2,Krhs) = 0
         ENDIF
         !tr(ji,jj,jk,jpdz1) = min(1., tr(ji,jj,jk,jpdz1))
         !tr(ji,jj,jk,jpdz2) = min(1., tr(ji,jj,jk,jpdz2))
     END_3D

         tr(:,:,:,jpdz1,Krhs) = min(1., tr(:,:,:,jpdz1,Krhs))
         tr(:,:,:,jpdz2,Krhs) = min(1., tr(:,:,:,jpdz2,Krhs))
     ...
     ! Calculate the max population growth rate for diazotroph  based on two diazo ecotypes

     DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, nksr )
            zprmaxdz(ji,jj,jk) = zprmaxdz(ji,jj,jk) + (tr(ji,jj,jk,jpdz1,Krhs)/(tr(ji,jj,jk,jpdz1,Krhs) + tr(ji,jj,jk,jpdz2,Krhs)) * zprmaxdz1(ji,jj,jk)) &
            &                    + (tr(ji,jj,jk,jpdz2,Krhs)/(tr(ji,jj,jk,jpdz1,Krhs) + tr(ji,jj,jk,jpdz2,Krhs)) * zprmaxdz2(ji,jj,jk))
            zprnfix(ji,jj,jk) = (tr(ji,jj,jk,jpdz1,Krhs)/(tr(ji,jj,jk,jpdz1,Krhs) + tr(ji,jj,jk,jpdz2,Krhs)) * zprnfix1(ji,jj,jk)) &
            &                    + (tr(ji,jj,jk,jpdz2,Krhs)/(tr(ji,jj,jk,jpdz1,Krhs) + tr(ji,jj,jk,jpdz2,Krhs)) * zprnfix2(ji,jj,jk))
            zprnutdz(ji,jj,jk) = (tr(ji,jj,jk,jpdz1,Krhs)/(tr(ji,jj,jk,jpdz1,Krhs) + tr(ji,jj,jk,jpdz2,Krhs)) * zprnutdz1(ji,jj,jk)) &
            &                    + (tr(ji,jj,jk,jpdz2,Krhs)/(tr(ji,jj,jk,jpdz1,Krhs) + tr(ji,jj,jk,jpdz2,Krhs)) * zprnutdz2(ji,jj,jk))
     END_3D
      ...
      DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, nksr )
         IF( etot_ndcy(ji,jj,jk) > 1.E-3 ) THEN
            ...
            zqfnmax = zqfnmax + ( MIN( qfemaxn , qfnmax-zqfnmax) * (tr(ji,jj,jk,jpfer, Kbb)**2 / ( tr(ji,jj,jk,jpfer, Kbb)**2 + kqfemaxn**2 ) ) )
      ...
      DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, nksr )
         IF( etot_ndcy(ji,jj,jk) > 1.E-3 ) THEN
            ...
            zpronmax = zpronmax * xqnpmin(ji,jj,jk) / qnpmin
            ...
            zqfpmax = zqfpmax + ( MIN( qfemaxp , qfpmax-zqfpmax) * ( tr(ji,jj,jk,jpfer,Kbb)**2 / ( tr(ji,jj,jk,jpfer,Kbb)**2 + kqfemaxp**2 ) ) )
      ...
      DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, nksr )
         IF( etot_ndcy(ji,jj,jk) > 1.E-3 ) THEN
            ...
            zpronmax = zpronmax * xqndmin(ji,jj,jk) / qndmin
            ...
            zqfdmax = zqfdmax + ( MIN( qfemaxd , qfdmax-zqfdmax) * (tr(ji,jj,jk,jpfer,Kbb)**2 / ( tr(ji,jj,jk,jpfer,Kbb)**2 + kqfemaxd**2 ) ) )
      ...
      ! Computation of the various production and uptake terms of diazotrophs
      ! Interactions between N and P are modeled according to the Chain Model 
      ! of Pahlow et al. (2009). Iron uptake is modeled following traditional
      ! Droop kinetics. When the quota is approaching the maximum achievable
      ! quota, uptake is downregulated according to a sigmoidal function 
      ...
      DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, nksr )
         IF( etot_ndcy(ji,jj,jk) > 1.E-3 ) THEN
            ...
            ! current time step
            ! --------------------------------------------------------------------
            zlimfac = xlimdiazo(ji,jj,jk) * zprchldz(ji,jj,jk) / ( zprmaxdz(ji,jj,jk) + rtrn )
            ...
            zpronmax = zpronmax * xqndzmin(ji,jj,jk) / qndzmin
            ...
            zmax = MAX(0., MIN(1., zratio**2 / (0.05**2 + zratio**2) ) )
            zpropmax = zprnutmax * zmax * xlimdzfe(ji,jj,jk) + (znfix(ji,jj,jk) / 122)
            ...
            zqfdzmax = zqfdzmax + ( MIN( qfemaxdz , qfdzmax-zqfdzmax) * (tr(ji,jj,jk,jpfer, Kbb)**2 / ( tr(ji,jj,jk,jpfer, Kbb)**2 + kqfemaxdz**2 ) ) )
      ...
      DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, nksr )
         IF( etot_ndcy(ji,jj,jk) > 1.E-3 ) THEN
            ...
            zprochld = MAX(zprochld, chlcmin * 12. * zprorcad(ji,jj,jk) )
               !  production terms for diazotroph ( chlorophyll )
            zdiaztot = ediazm(ji,jj,jk) / ( zmxl_chl(ji,jj,jk) + rtrn )
            zprod = rday * (zpronewdz(ji,jj,jk) + zproregdz(ji,jj,jk)) * zprchldz(ji,jj,jk) * xlimdiaz(ji,jj,jk)
            ...
            tr(ji,jj,jk,jpchd,Krhs) = tr(ji,jj,jk,jpchd,Krhs) + zprochldz * texcretdz
         ENDIF
      END_3D

      !   Update the arrays TRA which contain the biological sources and sinks
      DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, nksr )
        zpo4tot  = zpropo4n(ji,jj,jk) + zpropo4d(ji,jj,jk) + zpropo4p(ji,jj,jk) + zpropo4dz(ji,jj,jk)
        zpnewtot = zpronewn(ji,jj,jk) + zpronewd(ji,jj,jk) + zpronewp(ji,jj,jk) + zpronewdz(ji,jj,jk)
        zpregtot = zproregn(ji,jj,jk) + zproregd(ji,jj,jk) + zproregp(ji,jj,jk) + zproregdz(ji,jj,jk)
        ...
        &        + excretdz * zpropo4dz(ji,jj,jk) - texcretdz * zprodopdz(ji,jj,jk)
        ...
        tr(ji,jj,jk,jpcdz,Krhs) = tr(ji,jj,jk,jpcdz,Krhs)         &
           ...
           &                     - xpsino3 * zpronewdz(ji,jj,jk)   &
           &                     - xpsinh4 * zproregdz(ji,jj,jk)   &
           &                     - xpsinfix * znfix(ji,jj,jk)

        tr(ji,jj,jk,jpndz,Krhs) = tr(ji,jj,jk,jpndz,Krhs) + zprodztot * texcretdz
        tr(ji,jj,jk,jppdz,Krhs) = tr(ji,jj,jk,jppdz,Krhs) + ( zpropo4dz(ji,jj,jk) + zprodopdz(ji,jj,jk) ) * texcretdz
        ...
        tr(ji,jj,jk,jpdic,Krhs) = tr(ji,jj,jk,jpdic,Krhs) - zpptot  &
           ...
           &                     + xpsino3 * zpronewdz(ji,jj,jk) + xpsinh4 * zproregdz(ji,jj,jk)   &
           &                     + xpsinfix * znfix(ji,jj,jk)
    ...
    IF( iom_use( "tintpp" ) .OR. ( ln_check_mass .AND. kt == nitend .AND. knt == nrdttrc )  )  &
      & tpp = glob_sum( 'p6zprod', ( ( zprorcan(:,:,:) - xpsino3 * zpronewn(:,:,:) - xpsinh4 * zproregn(:,:,:)  &
              &                      + zprorcad(:,:,:) - xpsino3 * zpronewd(:,:,:) - xpsinh4 * zproregd(:,:,:)  &
              &                      + zprorcap(:,:,:) - xpsino3 * zpronewp(:,:,:) - xpsinh4 * zproregp(:,:,:)  &
              &                      + zprorcadz(:,:,:) - xpsino3 * zpronewdz(:,:,:) - xpsinh4 * zproregdz(:,:,:)  &
              &                      + xpsinfix * znfix(ji,jj,jk) ) * cvol(:,:,:) ) )
    ...
    IF( lk_iomput .AND.  knt == nrdttrc ) THEN
       ...
       ! Diazotrophy
       CALL iom_put( "GPPHYDZ"  , zprorcadz(:,:,:) * zfact * tmask(:,:,:) ) ! primary production by diazotrophs
       CALL iom_put( "PPPHYDZ"  , ( zprorcadz(:,:,:) - xpsino3 * zpronewdz(:,:,:) - xpsinh4 * zproregdz(:,:,:) ) * zfact * tmask(:,:,:) )  ! primary production by diazotroph
       CALL iom_put( "PPNEWDZ"  , zpronewdz(:,:,:) * zfact * tmask(:,:,:)   ) ! new primary production by diazotrophs
       CALL iom_put( "PFeDZ"    , zprofedz(:,:,:) * zfact * tmask(:,:,:)  ) ! biogenic iron production by  diazotrophs
       ...
       ! Diazotroph
       CALL iom_put( "MuDz"     , zprdiaz(:,:,:) * xlimdiaz(:,:,:) * tmask(:,:,:) ) ! Realized growth rate for diazotrophs
       CALL iom_put( "LDZlight" , zprdiaz(:,:,:) / (zprmaxdz(:,:,:) + rtrn) * tmask(:,:,:)   )
       CALL iom_put( "MunetDZ"  , ( tr(:,:,:,jpcdz,Krhs)/rfact2/(tr(:,:,:,jpcdz,Kbb)+ rtrn ) * tmask(:,:,:)) ) ! Realized growth rate for diazotrophs
       CALL iom_put( "Nfix"     , znfix(:,:,:) * rno3 * zfact * tmask(:,:,:) ) ! nitrogen fixation
       CALL iom_put( "Facul"     , Facul_out(:,:,:) * tmask(:,:,:) ) ! nitrogen fixation
        IF( iom_use("INTNFIX") ) THEN   ! nitrogen fixation rate in ocean (vertically integrated )
               zwork(:,:) = 0.
               DO jk = 1, jpkm1
                 zwork(:,:) = zwork(:,:) + znfix(:,:,jk) * rno3 * zfact * e3t(:,:,jk,Kmm)  * tmask(:,:,jk)
               ENDDO
               CALL iom_put( "INTNFIX" , zwork )
        ENDIF

          CALL iom_put( "evoclock" , evoclock(:,:,:) * tmask(:,:,:) )
          CALL iom_put( "bckevoclock" , bckevoclock(:,:,:) * tmask(:,:,:) )
          CALL iom_put( "zprmaxdz" , zprmaxdz(:,:,:) * tmask(:,:,:) )
          CALL iom_put( "zprmaxdz1" , zprmaxdz1(:,:,:) * tmask(:,:,:) )
          CALL iom_put( "zprmaxdz2" , zprmaxdz2(:,:,:) * tmask(:,:,:) )

       CALL iom_put( "TPP"     , ( zprorcap(:,:,:) + zprorcan(:,:,:) + zprorcad(:,:,:) + zprorcadz(:,:,:) ) * zfact * tmask(:,:,:)  )  ! total primary production
       CALL iom_put( "TPNEW"   , ( zpronewp(:,:,:) + zpronewn(:,:,:) + zpronewd(:,:,:) + zpronewdz(:,:,:) ) * zfact * tmask(:,:,:)  ) ! total new production
       CALL iom_put( "TPBFE"   , ( zprofep (:,:,:) + zprofen (:,:,:) + zprofed (:,:,:) + zprofedz(:,:,:) ) * zfact * tmask(:,:,:)  )  ! total biogenic iron production
   ...
   SUBROUTINE p6z_prod_init
      ...
      NAMELIST/namp6zprod/ pislopen, pislopep, pisloped, excretn, excretp, excretd,     &
         ...
         &                 thetandzm, pislopedz, excretdz,                &
         &                 qfemaxn, kqfemaxn, qfemaxp, kqfemaxp, qfemaxd, kqfemaxd, &
         &                 qfemaxdz, kqfemaxdz, evotime
      !!----------------------------------------------------------------------
...
902   IF( ios >  0 ) CALL ctl_nam ( ios , 'namp6zprod in configuration namelist' )
      ...
      IF(lwp) THEN                         ! control print
         ...
         WRITE(numout,*) '    P-I slope                                 pislopedz    =', pislopedz
         WRITE(numout,*) '    excretion ratio of diazotrophs            excretdz     =', excretdz
        WRITE(numout,*) '    Minimum Chl/N in diazotrophs               thetandzm =', thetandzm
        WRITE(numout,*) '    Timescale of diazotroph evolution          evotime      =', evotime
        IF( ln_bait ) THEN
         ...
         WRITE(numout,*) ' Luxury uptake QFe max                         qfemaxdz   =',qfemaxdz
         WRITE(numout,*) ' Luxury uptake QFe max shapefunction          kqfemaxdz   =',kqfemaxdz
      ENDIF
      ENDIF
      !
      r1_rday   = 1._wp / rday 
