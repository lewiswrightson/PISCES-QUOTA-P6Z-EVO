...
CONTAINS
   ...
   SUBROUTINE p4z_agg ( kt, knt, Kbb, Krhs )
      ...
      ! (1) catalysed by organic matter
                  tr(ji,jj,jk,jpafs,Krhs) = tr(ji,jj,jk,jpafs,Krhs) - zagg * ( tr(ji,jj,jk,jpafs,Kbb) &
                  &                        / ( tr(ji,jj,jk,jppoc,Kbb) + rtrn ) )
                  tr(ji,jj,jk,jpafb,Krhs) = tr(ji,jj,jk,jpafb,Krhs) + zagg * ( tr(ji,jj,jk,jpafs,Kbb) &
                  &                       / ( tr(ji,jj,jk,jppoc,Kbb) + rtrn ) )
      ! (2) autocatalytic
                  tr(ji,jj,jk,jpafs,Krhs) = tr(ji,jj,jk,jpafs,Krhs) - ( cfeagg * tr(ji,jj,jk,jpafs,Kbb) * zfact ) &
                  &                   - ( cfeagg2 * tr(ji,jj,jk,jpafb,Kbb) * xstep )
                  tr(ji,jj,jk,jpafb,Krhs) = tr(ji,jj,jk,jpafb,Krhs) + ( cfeagg * tr(ji,jj,jk,jpafs,Kbb) * zfact ) &
                  &                   + ( cfeagg2 * tr(ji,jj,jk,jpafb,Kbb) * xstep)
            ENDIF
         END_3D
      ELSE    ! ln_p5z
      ...
      ! As constants are per day, use zfact with shear and xstep without option
      ! to include some brownian aggregation and extra term is added to zagglfe
           zlfe = ( tr(ji,jj,jk,jplfe,Kbb) / 0.035 * 55.85 ) * 1e6 ! mg/m3 dust
           ! turbulent aggregation accounts for shear
            zagglfe = zlfe * ( ragglfe * zfact ) ! produes a per kt rate
            zagglfe = tr(ji,jj,jk,jplfe,Kbb) * zagglfe ! molLFe/L/kt
            !! aggregation with lfe and poc/goc not due to shear
            zagglfe2 = ( zlfe + ( ( tr(ji,jj,jk,jppoc,Kbb) + tr(ji,jj,jk,jpgoc,Kbb) ) * 24.E6 ) ) &
            &        *  ( ragglfe * aggrat ) * xstep ! produces a per kt rate
            zagglfe2 = tr(ji,jj,jk,jplfe,Kbb) * zagglfe2 ! molLFe/L/kt
      ! turbulent dissaggration of LFe aggregates
           zdislfea = tr(ji,jj,jk,jplfa,Kbb) * rdislfea * zfact ! molLFea/L/kt
      ! update SMS
           tr(ji,jj,jk,jplfe,Krhs) = tr(ji,jj,jk,jplfe,Krhs) - zagglfe - zagglfe2 + zdislfea
           tr(ji,jj,jk,jplfa,Krhs) = tr(ji,jj,jk,jplfa,Krhs) + zagglfe + zagglfe2 - zdislfea
         END_3D
         ENDIF
      !
