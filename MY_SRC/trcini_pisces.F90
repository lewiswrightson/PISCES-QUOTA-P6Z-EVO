...
CONTAINS
   ...
   SUBROUTINE trc_ini_pisces( Kmm )
      ...
      CALL trc_nam_pisces
      !
      IF( ln_p4z .OR. ln_p5z .OR. ln_p6z ) THEN  ;   CALL p4z_ini( Kmm )   !  PISCES
   ...
   SUBROUTINE p4z_ini( Kmm )
      ...
      ENDIF
      IF( ln_p6z ) THEN
         ! PISCES-QUOTA Explicit diazotrophy part
      ...
      DO jn = 1, jptra
        ...
        IF( cltra == 'DZ1'      )   jpdz1 = jn      !: Proportion of croco 28T
        IF( cltra == 'DZ2'      )   jpdz2 = jn      !: Proportion of croco 32T
      END DO
      ...
      IF( .NOT.ln_rsttr ) THEN  
         ...
         ENDIF
         IF( ln_felith) THEN
            tr(:,:,:,jplfe,Kmm) = bioma0 * 1.e-6
            tr(:,:,:,jplfa,Kmm) = bioma0 * 1.e-6
         ENDIF
         IF( ln_feauth) THEN
            tr(:,:,:,jpafs,Kmm) = bioma0 * 1.e-6
            tr(:,:,:,jpafb,Kmm) = bioma0 * 1.e-6
         ENDIF
