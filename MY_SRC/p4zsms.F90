...
908   IF( ios >  0 )   CALL ctl_nam ( ios , 'nampismass in configuration namelist' )
   ...
   SUBROUTINE p4z_rst( kt, Kbb, Kmm, cdrw )
        ...
        ! PISCES-QUOTA Explicit Diazotroph (P6Z) specific part
         IF( ln_p6z ) THEN
            ...
            ELSE
               vnfmax_dz(:,:,:) = 1.
            ENDIF
            IF( iom_varid( numrtr, 'evoclock', ldstop = .FALSE. ) > 0 ) THEN
               CALL iom_get( numrtr, jpdom_auto, 'evoclock' , evoclock(:,:,:)  )
               evoclock(:,:,:) = max(0., evoclock(:,:,:))
            ELSE
               evoclock(:,:,:) = 0.
            ENDIF
            IF( iom_varid( numrtr, 'bckevoclock', ldstop = .FALSE. ) > 0 ) THEN
               CALL iom_get( numrtr, jpdom_auto, 'bckevoclock' , bckevoclock(:,:,:) )
               bckevoclock(:,:,:) = max(0., bckevoclock(:,:,:))
            ELSE
               bckevoclock(:,:,:) = 0.
            ENDIF
         ENDIF
      ...
      ELSEIF( TRIM(cdrw) == 'WRITE' ) THEN
         ...
         IF( ln_p5z .OR. ln_p6z) CALL iom_rstput( kt, nitrst, numrtw, 'sizep', sizep(:,:,:) )  ! Size of picophytoplankton
         IF( ln_p6z ) CALL iom_rstput( kt, nitrst, numrtw, 'sizedz', sizedz(:,:,:) )  ! Size of diazotroph phytoplankton
         IF( ln_p6z ) CALL iom_rstput( kt, nitrst, numrtw, 'evoclock', evoclock(:,:,:) )! Forwards Evolution Clock
         IF( ln_p6z ) CALL iom_rstput( kt, nitrst, numrtw, 'bckevoclock', bckevoclock(:,:,:) )! Backwards Evolution Clock
      ENDIF
      !
   ...
   SUBROUTINE p4z_chk_mass( kt, Kmm )
      ...
      IF( iom_use( "pno3tot" ) .OR. ( ln_check_mass .AND. kt == nitend )  ) THEN
         ...
         IF( ln_p6z ) THEN
            zwork(:,:,:) =    tr(:,:,:,jpno3,Kmm) + tr(:,:,:,jpnh4,Kmm) + tr(:,:,:,jpnph,Kmm)   &
               &          +   tr(:,:,:,jpndi,Kmm) + tr(:,:,:,jpnpi,Kmm) + tr(:,:,:,jpndz,Kmm) &
               &          +   tr(:,:,:,jppon,Kmm) + tr(:,:,:,jpgon,Kmm) + tr(:,:,:,jpdon,Kmm)   &
               &          + ( tr(:,:,:,jpzoo,Kmm) + tr(:,:,:,jpmes,Kmm) ) * no3rat3
         ENDIF
      ...
      IF( iom_use( "ppo4tot" ) .OR. ( ln_check_mass .AND. kt == nitend )  ) THEN
         ...
         IF( ln_p6z ) THEN
            zwork(:,:,:) =    tr(:,:,:,jppo4,Kmm) + tr(:,:,:,jppph,Kmm) &
               &          +   tr(:,:,:,jppdi,Kmm) + tr(:,:,:,jpppi,Kmm) + tr(:,:,:,jppdz,Kmm) &
               &          +   tr(:,:,:,jppop,Kmm) + tr(:,:,:,jpgop,Kmm) + tr(:,:,:,jpdop,Kmm)   &
               &          + ( tr(:,:,:,jpzoo,Kmm) + tr(:,:,:,jpmes,Kmm) ) * po4rat3
         ENDIF
      ...
      IF( iom_use( "pfertot" ) .OR. ( ln_check_mass .AND. kt == nitend )  ) THEN
       ...
       IF( ln_p5z ) THEN
         zwork(:,:,:) =   tr(:,:,:,jpfer,Kmm) + tr(:,:,:,jpnfe,Kmm) + tr(:,:,:,jpdfe,Kmm)   &
            &         +   tr(:,:,:,jppfe,Kmm) + tr(:,:,:,jpbfe,Kmm) + tr(:,:,:,jpsfe,Kmm) &
            &         + ( tr(:,:,:,jpzoo,Kmm) * feratz + tr(:,:,:,jpmes,Kmm) ) * feratm 
       ENDIF
       IF( ln_p6z ) THEN
         zwork(:,:,:) =   tr(:,:,:,jpfer,Kmm) + tr(:,:,:,jpnfe,Kmm) + tr(:,:,:,jpdfe,Kmm)   &
