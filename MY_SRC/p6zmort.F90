...
CONTAINS
   ...
   SUBROUTINE p6z_mort( kt, Kbb, Krhs )
      ...
      CALL p6z_mort_diaz( Kbb, Krhs )            ! diazotrophs
   ...
   SUBROUTINE p6z_mort_diaz( Kbb, Krhs )
      !!---------------------------------------------------------------------
      !!                     ***  ROUTINE p6z_mort_diaz  ***
      !!
      !! ** Purpose :   Compute the mortality terms for diazotrophys
      !!
      !! ** Method  : - Both quadratic and simili linear mortality terms
      !!---------------------------------------------------------------------
      INTEGER, INTENT(in) ::   Kbb, Krhs  ! time level indices
      ...
      !!---------------------------------------------------------------------
      !
      IF( ln_timing )   CALL timing_start('p6z_mort_diaz')
      !
      prodcal(:,:,:) = 0.  !: calcite production variable set to zero
      DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, jpkm1)
      ...
      !
       ...
       ENDIF
      !
      IF( ln_timing )   CALL timing_stop('p6z_mort_diaz')
      !
   END SUBROUTINE p6z_mort_diaz
