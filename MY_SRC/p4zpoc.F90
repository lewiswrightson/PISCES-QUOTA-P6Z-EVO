MODULE p4zpoc
   ...
   REAL(wp), PUBLIC ::   xremafs    !: remineralisation rate of small authigenicFe
   REAL(wp), PUBLIC ::   xremafb    !: remineralisation rate of big authigenicFe
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   dust    !: dustfields
...
CONTAINS
   ...
   SUBROUTINE p4z_poc( kt, knt, Kbb, Kmm, Krhs )
     ...
     ! The standard PISCES part
      DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, jpkm1)
         ...
         zofer2 = zremig * tr(ji,jj,jk,jpbfe,Kbb)
           IF ( ln_felith) THEN
                  zdust = ( tr(ji,jj,jk,jplfe,Kbb) + tr(ji,jj,jk,jplfa,Kbb) ) / 0.035 * 55.85
           ELSE
                  zdust = dust(ji,jj) / ( wdust / rday ) * tmask(ji,jj,jk) &
                  &  * EXP( -gdept(ji,jj,jk,Kmm) /( 250 ) )
           ENDIF
           IF ( ln_dustrfe ) THEN
     ...
     ! remineralisation of authigenic Fe
                  afe1 = tr(ji,jj,jk,jpafs,Kbb) * xremafs * xstep
                  afe2 = tr(ji,jj,jk,jpafb,Kbb) * xremafb * xstep
!                  zfafs(ji,jj,jk)   = afe1
!                  zfafb(ji,jj,jk)   = afe2
                  tr(ji,jj,jk,jpafs,Krhs) = tr(ji,jj,jk,jpafs,Krhs) - afe1 +afe2
                  tr(ji,jj,jk,jpafb,Krhs) = tr(ji,jj,jk,jpafb,Krhs) - afe2
                  tr(ji,jj,jk,jpfer,Krhs) = tr(ji,jj,jk,jpfer,Krhs) + afe1
           END_3D
        ENDIF
   ...
   SUBROUTINE p4z_poc_init
      ...
      NAMELIST/nampispoc/ xremip , jcpoc  , rshape,  &
         ...
         &                ln_dustrfe, kdust, xremafs, xremafb
      !!----------------------------------------------------------------------
      !
      IF(lwp) THEN
         WRITE(numout,*)
