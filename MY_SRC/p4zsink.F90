MODULE p4zsink
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::  sinklfeb  !: POC
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::  sinklfab  !: POC

   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::  sinkafsb  !: POC
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::  sinkafbb  !: POC
...
CONTAINS
   ...
   SUBROUTINE p4z_sink ( kt, knt, Kbb, Kmm, Krhs )
      ...
      IF( ln_bait ) THEN
       ...
       IF( ln_felith ) THEN
         sinklfeb (:,:,:) = 0.e0
         sinklfab(:,:,:) = 0.e0
         CALL trc_sink( kt, Kbb, Kmm, wsbio5, sinklfeb, jplfe, rfact2 )
         CALL trc_sink( kt, Kbb, Kmm, wsbio6, sinklfab, jplfa, rfact2 )
       ENDIF

       IF( ln_feauth ) THEN
         sinkafsb (:,:,:) = 0.e0
         sinkafbb(:,:,:) = 0.e0
         CALL trc_sink( kt, Kbb, Kmm, wsbio7, sinkafsb, jpafs, rfact2 )
         CALL trc_sink( kt, Kbb, Kmm, wsbio8, sinkafbb, jpafb, rfact2 )
       ENDIF
     ENDIF
     ...
     IF( lk_iomput .AND.  knt == nrdttrc ) THEN
       ...
       ! 
       IF( ln_felith ) THEN
       CALL iom_put( "EPLFE100", ( sinklfeb(:,:,ik100) + sinklfab(:,:,ik100) ) * zfact * tmask(:,:,1) ) !
       CALL iom_put( "EXPLFE", ( sinklfeb(:,:,:) + sinklfab(:,:,:) ) * zfact * tmask(:,:,:) ) !
       ENDIF
       IF( ln_feauth ) THEN
       CALL iom_put( "EPAFE100", ( sinkafsb(:,:,ik100) + sinkafbb(:,:,ik100) ) * zfact * tmask(:,:,1) ) !
       CALL iom_put( "EXPAFE", ( sinkafsb(:,:,:) + sinkafbb(:,:,:) ) * zfact * tmask(:,:,:) ) !
       ENDIF
       !
      ENDIF
      !
   ...
   INTEGER FUNCTION p4z_sink_alloc()
      ...
      IF( ln_p5z .OR. ln_p6z   ) ALLOCATE( sinkingn(jpi,jpj,jpk), sinking2n(jpi,jpj,jpk)   ,     &
         ...
         IF( ln_felith ) ALLOCATE( sinklfeb(jpi,jpj,jpk), sinklfab(jpi,jpj,jpk), STAT=ierr(3) )
         IF( ln_feauth ) ALLOCATE( sinkafsb(jpi,jpj,jpk), sinkafbb(jpi,jpj,jpk), STAT=ierr(4) )
      !
